...
async function fetchPopularMovies() {
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/movie/popular?api_key=${apiKey}&language=en-US`);
        const movies = await response.json(); // tu codigo aqui: realiza una solicitud para obtener las películas populares

        displayMovies(movies.results); // y llama a displayMovies con los resultados
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        ...
        try {
            ...
            const movies = await response.json();// tu codigo aqui: realiza una solicitud para buscar películas

            displayMovies(movies.results); // y llama a displayMovies con los resultados de la búsqueda
